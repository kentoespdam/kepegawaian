...
This project is indexed by GitNexus as **kepegawaian** (19716 symbols, 44416 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
This project is indexed by **Graphify** ([Graphify-Labs/graphify](https://github.com/Graphify-Labs/graphify)) — a local-first knowledge graph tool that maps code, docs, and config into a structured, queryable graph.

### Setup

Graphify CLI is installed via `uv tool install graphifyy`. The skill lives at `.agents/skills/graphify/SKILL.md`.

### Outputs

| Output | Location |
|--------|----------|
| Interactive HTML | `graphify-out/graph.html` |
| JSON graph data | `graphify-out/graph.json` |
