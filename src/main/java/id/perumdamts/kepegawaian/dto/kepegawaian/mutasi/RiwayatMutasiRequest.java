...
public class RiwayatMutasiRequest extends CommonPageRequest {
    ...
    public Specification<RiwayatMutasi> getSpecification() {
        ...
        Specification<RiwayatMutasi> jenisMutasiSpec = Objects.isNull(jenisMutasi) ? null :
                (root, query, criteriaBuilder) -> criteriaBuilder.equal(root.get("jenisMutasi"), jenisMutasi);
        ...
        return Specification.where(pegawaiSpec).and(riwayatSkSpec).and(jenisMutasiSpec).and(skSpec).and(organisasiSpec).and(namaOrganisasiSpec)
