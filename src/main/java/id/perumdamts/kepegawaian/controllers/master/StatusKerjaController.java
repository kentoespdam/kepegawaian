...
public class StatusKerjaController {
    ...
    @GetMapping
    public ResponseEntity<?> index(@ParameterObject StatusKerjaRequest request) {
    ...
    @PostMapping
    public ResponseEntity<?> save(@Valid @RequestBody StatusKerjaPostRequest request, Errors errors) {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @PostMapping("/batch")
    public ResponseEntity<?> saveBatch(@Valid @RequestBody List<StatusKerjaPostRequest> requests, Errors errors) {
        if (errors.hasErrors()) return ErrorResult.build(errors);
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @PutMapping("/{id}")
    public ResponseEntity<?> update(@PathVariable Long id, @Valid @RequestBody StatusKerjaPutRequest request, Errors errors) {
        if (errors.hasErrors()) return ErrorResult.build(errors);
