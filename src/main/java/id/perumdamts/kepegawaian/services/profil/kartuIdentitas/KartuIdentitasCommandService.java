...
public class KartuIdentitasCommandService {
    ...
    public Long create(KartuIdentitasPostRequest request) {
        ...
        Optional<KartuIdentitas> existing = repository.findAnyByUniqueKey(request.getNik(), request.getJenisKartuId(), request.getNomorKartu());
    ...
    public KartuIdentitas seedFromBiodata(Biodata biodata) {
        JenisKitas jenisKtp = jenisKitasRepository.findById(1L).orElseThrow(() -> new NotFoundException(UNKNOWN_JENIS_KARTU));
