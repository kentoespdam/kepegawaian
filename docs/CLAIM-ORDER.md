...
- [ ] **kepegawaian-2nd** (P2) — Ekstrak `CutiPegawaiSelects` (shared `Field<?>[]`)
  - Prasyarat semua fix cuti. WAJIB pertahankan kontrak string-alias (`org_*`, `jab_*`, `jc_*`, `sjc_*`, `pic_*`).
- [ ] **kepegawaian-d23** (P2) — Fix inbox N+1 via `QUALIFY ROW_NUMBER()` *(blocked-by 2nd)*
...
- [ ] **kepegawaian-4aw** (P2) — `CutiApprovalChainResponse.refCuti` → `CutiPengajuanMiniResponse`
  - Buang nesting berlebih; factory `from()` panggil `CutiPengajuanMiniResponse.from`.
- [ ] **kepegawaian-9j0** (P3) — Hapus secondary N+1 di `CutiPengajuanQueryRepository.pageQuery`
...
- [ ] **kepegawaian-rvw** (P2, child of be8) — Hapus `services/setupMaster/` + `SetupMasterController`
...
- [ ] **kepegawaian-jyh** (P3) — Audit FK-duplikat semua read-DTO (§4c)
...
- [ ] **kepegawaian-a66** (P3) — Migrasi DTO/read model sisi Query CQRS ke Java record
  - Anak (DTO trivial, sudah digrill): **2fl** `AlatKerjaQuery`, **9iy** `SanksiMiniResponse`, **a66.1** `ProfilUpdateDetail<T>`.

---

## Fase 4 — Pegawai read-side: dekomposisi findById

- [ ] **kepegawaian-5ca** (P2) — Dekomposisi `PegawaiQueryRepository.findById` (625 baris → §4)
  ...
  - `detailFields()` **standalone** (subset kolom beda dari `pegawaiResponseFields`). `skList` di-pass sbg argumen; `findRiwayatSkList`+`getLastFromResponseList` **tetap di repo**. Subquery korelasi `grade_level_nama` **dipertahankan**. Kontrak string-alias WAJIB dijaga. 1 pemanggil (`PegawaiQueryService`), pure decomposition.

## Sudah digrill — DTO→record (child of a66)

- [ ] **kepegawaian-2fl** — `AlatKerjaQuery` (`@Data`→record). Record-safe: hanya `fetchOptionalInto`, tak ada mutasi setter eksternal.
- [ ] **kepegawaian-9iy** — `SanksiMiniResponse` (`@AllArgsConstructor @Data`→record). Record-safe; `from(Sanksi)` dipertahankan.
- [ ] **kepegawaian-a66.1** — `ProfilUpdateDetail<T>`→record + buang kebocoran entity JPA (field→`ProfileUpdateQuery`, `build()` dijaga, tambah `ProfileUpdateQuery.from(entity)`; jalur baca tetap JPA/Envers).
