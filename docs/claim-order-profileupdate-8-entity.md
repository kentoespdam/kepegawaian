...
| W4c ⏳ | **Koordinasi FE**: cek pemakaian `POST /profil/lampiran/accept` di `kepegawaian-fe` sebelum hapus; siapkan nota breaking change | (koordinasi) — **NOTA BREAKING CHANGE**: endpoint `POST /profil/lampiran/accept` + DTO `LampiranProfilAcceptRequest` dihapus. Approval lampiran kini via `PUT /profil/profil-update/{id}`. FE yang masih memanggil endpoint lama → 404. |
