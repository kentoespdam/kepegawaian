...
- **Profil (2026-06-25):** `profil` kini mereplikasi struktur package `master`: repositori JPA di `repositories/profil/jpa/`, repositori JOOQ telah berada di `repositories/profil/jooq/`, dan 8 mapper baru di `mapper/profil/<sub>/`. Modul legacy `lampiranProfil` masih mempertahankan pola `*Service`+`*Impl`, dan `profilUpdate` masih `*Service`+`*Impl`+3 `*ApprovalService`.
