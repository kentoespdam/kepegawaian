...
Potongan ad-hoc di luar komponen standar (pinjaman koperasi, bank, zakat, iuran, dsb.) yang di-upload ke sebuah **Batch Gaji** lewat **Form Potongan Gaji**. Setiap sel terisi menjadi satu baris **Proses Master Gaji**: `kode = ADD_<KATEGORI>` (dari label kolom form — huruf besar, karakter non-alfanumerik jadi `_`), `jenisGaji = POTONGAN`, `urut = 99`. Berlaku hanya pada **Batch Gaji** berstatus **Verifikasi Tahap 2** (window upload). Upload bersifat **replace**: unggah ulang menghapus semua baris `ADD_%` milik batch itu lalu memasukkan ulang; unggahan ditolak seluruhnya bila ada NIPAM yang tidak ada di **Master Gaji** batch tersebut atau NIPAM duplikat antar-sheet. Nilai kosong atau nol tidak menghasilkan baris; nilai negatif menolak file. Menerapkannya memicu **recalculate** ulang total; membatalkannya = **rollback** (hapus semua baris `ADD_%` lalu nol-kan total).
_Avoid_: kode `adhoc` (istilah legacy — pakai kode `ADD_<KATEGORI>`), "upload menambah tanpa menghapus" (accumulate).

**Form Potongan Gaji** (Daftar Potongan Gaji Pegawai):
Workbook Excel resmi multi-sheet — satu sheet per **organisasi** (DIREKSI, BAGIAN KEUANGAN, CABANG …). Tiap sheet: header kop (baris 1–7), judul + label organisasi, header tabel di baris 9 (`NO. | NAMA | NIPAM | GAJI | POTONGAN… | JUMLAH POTONGAN | GAJI BERSIH`) dan baris 10 berisi label kategori potongan (mulai kolom E). Satu pegawai = satu baris per sheet organisasinya; satu pegawai bisa punya banyak kategori potongan sekaligus.
